...
@Component({
  ...
  templateUrl: './categorieB.component.html',
  styleUrl: './categorieB.component.scss',
})
export class CategorieBComponent implements OnInit {
  // 🔥 Source de vérité = stock
  ligneStocksCategorieB$!: Observable<LigneStock[]>;

  constructor(private produitService: CartProduitService) {}

  ngOnInit(): void {
    // 🔥 on part du stock uniquement
    this.ligneStocksCategorieB$ = this.produitService.getStocks().pipe(
      map((stocks) => stocks.flatMap((stock) => stock.lignesStock)),
      map((lignesStock) =>
        lignesStock.filter((ls) => ls.produit.categorieId === 2),
      ),
    );
  }
